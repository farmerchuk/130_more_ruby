...
class Crypto
  ...
  def plaintext_segments
    ...
    loop do
      break if plaintext.empty?
      result << plaintext.slice!(0, size)
    end
  ...
  def create_ciphertext
    result = Array.new(size) { Array.new }

    result.cycle do |ary|
      break if plaintext.empty?
      ary << plaintext.slice!(0)
    end

    result.map! { |segment| segment.join('') }
    result
  end
end
