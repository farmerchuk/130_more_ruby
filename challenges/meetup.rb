...
class Meetup
  TEENTHS = (13..19)
  WEEKS = { first: 1, second: 2, third: 3, fourth: 4 }
  ...
  def day(weekday, schedule)
    wkday_meth = weekday.to_s << '?'

    case schedule
    when :teenth then find_teenth_date(wkday_meth, schedule)
    when :first, :second, :third, :fourth then find_date(wkday_meth, schedule)
    when :last then find_last_date(wkday_meth, schedule)
    end
  ...
  def find_teenth_date(wkday_meth, schedule)
    loop do
      if date.send(wkday_meth) && TEENTHS.include?(date.day)
        break
      end
      self.date = date.next
    end
  ...
  def find_date(wkday_meth, schedule)
    day_count = 0

    loop do
      day_count += 1 if date.send(wkday_meth)
      break if date.send(wkday_meth) && day_count == WEEKS[schedule]
      self.date = date.next
    end
  end

  def find_last_date(wkday_meth, schedule)
    day_count = 0
    latest_date = date
    next_month = date.next_month

    loop do
      break if date.month == next_month.month
      if date.send(wkday_meth)
        day_count += 1
        latest_date = date
      end
      self.date = date.next
